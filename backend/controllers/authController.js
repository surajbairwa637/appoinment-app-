...
const signToken = async (id, role, name ,email,admissionStatus) => {
  return await jwt.sign({ id, role, name,email,admissionStatus }, process.env.JWT_SECRET, {
...
exports.adminLogin = catchAsync(async (req, res, next) => {
  ...
  if (email !== process.env.ADMIN_EMAIL || password !== process.env.ADMIN_PASSWORD) {
    return next(new AppError('Incorrect admin email or password'));
  }

  const token = await signToken('admin', 'admin', 'Admin', email, true);
